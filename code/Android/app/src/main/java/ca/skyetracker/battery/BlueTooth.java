...
public class BlueTooth extends IntentService {
    ...
    public BlueTooth() {
        super("BlueTooth");
        mConnectedThread = new ConnectedThread("BlueToothWorker");
    }
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if ("ca.skyetracker.battery.Connect".equalsIgnoreCase(intent.getAction())) {
            try {
                if (SetupBluetooth()) {
                    mConnectedThread.start();
                    LocalBroadcastManager.getInstance(this.getBaseContext()).registerReceiver(mCommandReceiver, new IntentFilter("ca.skyetracker.battery.Write"));
                } else {
        ...
        if ("ca.skyetracker.battery.Disconnect".equalsIgnoreCase(intent.getAction())) {
            Stop();
        }
    }
    ...
    private void updateBluetoothStatus() {
        Intent commandIntent = new Intent("ca.skyetracker.battery.bluetooth", null, getBaseContext(), BlueTooth.class);
    ...
    private boolean SetupBluetooth() throws InterruptedException {
        ...
        while (!bluetoothConnected) {
            ...
            if (addresses.isEmpty()) {
                ...
                mHandler.post(new DisplayToast(this, "...App not paired with this device..."));
                Thread.sleep(2000);
            } else {
                for (String address : addresses) { // find find available HC-06 bluetooth device
                    ...
                    try {
                        ...
                        mHandler.post(new DisplayToast(this, "...Failed to connect to device..."));
                        try {
                            btSocket.close();
